...
export default function AdminStockInReportPage() {
  ...
  const load = async () => {
    try {
      setData(await api.getStockInReport(from || undefined, to || undefined));
    } catch {} finally {
      setLoading(false);
    }
  };
  ...
  const totalValue = data.reduce((sum, d) => sum + parseFloat(d.purchasePrice || "0") * (d.qty || 0), 0);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2"><Download className="w-6 h-6" /> Laporan Stok Masuk</h1>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card className="border-border/50">
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-bold mt-1">{totalQty} unit</p>
          </CardContent>
        </Card>
        <Card className="border-border/50">
          <CardContent className="p-6">
            <p className="text-sm text-muted-foreground">Total Nilai Pembelian</p>
            <p className="text-2xl font-bold mt-1 text-primary">{formatRupiah(totalValue)}</p>
          </CardContent>
        </Card>
      </div>
      <Card className="border-border/50">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border/50 text-muted-foreground">
                ...
                <th className="text-right py-3 px-4 font-medium">Harga Beli</th>
              </tr>
            </thead>
            <tbody>
              {data.length === 0 ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="border-b border-border/30 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="py-3 px-4 text-right">{formatRupiah(parseFloat(item.purchasePrice || "0"))}</td>
                  </tr>
                ))
              )}
            </tbody>
