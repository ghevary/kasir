...
export default function AdminFinancialReportPage() {
  ...
  const load = async () => {
    try { setData(await api.getFinancialReport(from || undefined, to || undefined)); } catch {} finally { setLoading(false); }
  };
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2"><Coins className="w-6 h-6" /> Laporan Keuangan</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-border/50"><CardContent className="p-6"><p className="text-sm text-muted-foreground">Cash</p><p className="text-2xl font-bold mt-1 text-emerald-400">{formatRupiah(data.summary?.totalCash || 0)}</p></CardContent></Card>
        <Card className="border-border/50"><CardContent className="p-6"><p className="text-sm text-muted-foreground">QRIS</p><p className="text-2xl font-bold mt-1 text-blue-400">{formatRupiah(data.summary?.totalQris || 0)}</p></CardContent></Card>
